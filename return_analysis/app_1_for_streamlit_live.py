...
def load_data():
    try:
        ...
        # FIX: Ekhane 'Quantity' column tao read kora hocche
    ...
    if st.button("⚡ Get Metrics & Solution", type="primary", use_container_width=True):
        if selected_sku == "-- Select --":
            ...
            with st.spinner("⏳ Analyzing Data & Consulting Gemini..."):
                ...
                # Target Return Data filter
                target_data = df_with_comments[df_with_comments['SKU'] == selected_sku]
                
                # 📊 Sales Metrics (Gross Units & GMV) - from sales file
                ...
                # 📊 Return Metrics (Quantity) - STRICTLY from return file
                if 'Quantity' in target_data.columns:
                    return_units = int(target_data['Quantity'].sum())
                elif 'quantity' in [c.lower() for c in target_data.columns]:
                    # Case-insensitive check jodi column er nam ektu onnorokom thake
                    qty_col = [c for c in target_data.columns if c.lower() == 'quantity'][0]
                    return_units = int(target_data[qty_col].sum())
                else:
                    return_units = len(target_data) # Safe fallback jodi Quantity column missing thake
