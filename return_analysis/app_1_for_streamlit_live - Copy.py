...
def load_data():
    try:
        ...
        # full_returns_df এ 'Quantity' কলামও রিড করব রিটার্ন কাউন্ট করার জন্য
    ...
    if st.button("⚡ Get Metrics & Solution", type="primary", use_container_width=True):
        if selected_sku == "-- Select --":
            ...
            with st.spinner("⏳ Analyzing Data & Consulting Gemini..."):
                ...
                # Sales Metrics (earn_more.xlsx থেকে Gross Units এবং GMV)
                ...
                # Return Metrics (সরাসরি fk_mars_return.xlsx এর 'Quantity' কলাম থেকে Sum করা হলো)
                sku_return_data = full_returns_df[full_returns_df['SKU'] == selected_sku]
                if not sku_return_data.empty:
                    return_units = int(sku_return_data['Quantity'].sum())
                else:
                    return_units = 0
                
                # Return Percentage বা রিটার্ন রেট ক্যালকুলেশন
                ...
                m3.metric(label="Return Units (from Flipkart Sheet)", value=f"{return_units}", delta_color="inverse")
