...
# ==========================================
# 2. DATA LOADING (FIXED DATA EXPLOSION)
# ==========================================
@st.cache_resource
def load_data():
    try:
        ...
        sales_df = pd.read_excel(earn_more_path, usecols=lambda c: c in sales_needed_cols)
        sales_df = sales_df.rename(columns={'SKU ID': 'SKU'})

        returns_df = pd.read_excel(fk_mars_path, usecols=lambda c: c in ['SKU', 'Comments', 'Quantity'])
        
        # Dropdown ebong comment er jonno merge korchi
        df = pd.merge(returns_df, sales_df, on='SKU', how='left')
        df_with_comments = df.dropna(subset=['Comments'])
        
        # Ekhane 'returns_df' (original) ta extra return korchi pure math er jonno
        return sales_df, returns_df, df_with_comments
    except Exception as e:
        st.error(f"❌ Excel File Load Error: {e}")
        return None, None, None

sales_df, returns_df, df_with_comments = load_data()
    ...
    if st.button("⚡ Get Metrics & Solution", type="primary", use_container_width=True):
        if selected_sku == "-- Select --":
            ...
            with st.spinner("⏳ Analyzing Data & Consulting Gemini..."):
                ...
                # 📊 Sales Metrics (Strictly from unmerged sales data)
                ...
                if not sku_sales_data.empty:
                    ...
                    gross_units, total_gmv = 0, 0.0
                
                # 📊 Return Metrics (Strictly from UNMERGED raw return data)
                # Ei line tar jonnoi 732 er bodole ekdom correct 23 asbe!
                sku_raw_returns = returns_df[returns_df['SKU'] == selected_sku]
                
                if 'Quantity' in sku_raw_returns.columns:
                    return_units = int(sku_raw_returns['Quantity'].sum())
                elif 'quantity' in [c.lower() for c in sku_raw_returns.columns]:
                    qty_col = [c for c in sku_raw_returns.columns if c.lower() == 'quantity'][0]
                    return_units = int(sku_raw_returns[qty_col].sum())
                else:
                    return_units = len(sku_raw_returns)
                ...
                m3.metric(label="Return Qty", value=f"{return_units}", delta_color="inverse")
